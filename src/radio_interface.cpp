...
#define IV_SIZE 4 // 32 bits = 4 bytes
...
static uint8_t iv[4] = {0x00, 0x00, 0x00, 0x00};
...
void encrypt(uint8_t* buf){
  ...
  */
  ...
  memmove(&buf[SPACEPACKET_ENCODED_HEADER_SIZE + IV_SIZE], &buf[SPACEPACKET_ENCODED_HEADER_SIZE], pck_length);
  memcpy(&buf[SPACEPACKET_ENCODED_HEADER_SIZE], iv, IV_SIZE);
  // TODO: is the data_field_encrypted necessary if the apid tells us that it is?
}
