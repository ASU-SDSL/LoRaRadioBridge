...
static AES_ctx* ctx; // initiallized once a command
static uint32_t encryption_length = 2; // in bytes, so 16 bits. Not sure if it is possible to reduce to 11 bits
...
// key CANNOT be stored on this file, also the key was generated by CHATGPT so change it
static uint8_t iv[16] = {
 ...
 // needs to be changed everytime a command is sent
...
bool isEncryptedCommand() {
  // header = spacepacket_header_t, will this file be made public?
  switch (header.apid)
  {
  case 5:
    return true;
...
void encrypt(uint16_t id_to_encrypt){
  // TODO: fix this mess

  AES_init_ctx_iv(ctx, key, iv); // initialize the context calling
  AES_CTR_xcrypt_buffer(ctx, id_to_encrypt, encryption_length); // encrypts the 
}
...
void decodeAndTransmit() {
  ...
  if(isEncryptedCommand()){ encrypt(header.apid); }
